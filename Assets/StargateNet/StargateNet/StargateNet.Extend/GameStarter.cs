    ...
    public class GameStarter : MonoBehaviour
    {
        ...
        private void OnGUI()
        {
            ...
            if (_showConnectBtn && GUI.Button(new Rect(10, 10, 100, 90), "Server"))
            {
                ...
                var galaxy = SgNetwork.StartAsServer(serverPort);
                mode = 2;
            }
            ...
            {
                _showConnectBtn = false;
                var galaxy = SgNetwork.StartAsServerAndBot(serverPort);
                mode = 1;
            }
            ...
            if (!_showConnectBtn && SgNetwork.Instance.monitor != null)
            {
                ...
                string textToDisplay = $"Tick: {monitor.tick}\n" +
                                       ...
                                       $"InKBps: {SgNetwork.Instance.galaxies[0].InKBps}KB/s\n" +
                                       $"OutKBps: {SgNetwork.Instance.galaxies[0].OutKBps}KB/s\n" +
                                       $"InterpolationDelay: {SgNetwork.Instance.galaxies[0].InterpolateDelay * 1000f}ms\n" +
            ...
            {
                if (refs.Count > 0)
                    SgNetwork.Instance.galaxies[0].NetworkDestroy(refs.Dequeue());
            }
        }
    }
}